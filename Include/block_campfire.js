class campfire extends activeblock {
    ...
    acceptsinput(item) {
        ...
        switch (item.name) {
            ...
            case 'deaddeer':
            case 'deadwolf':
            case 'deadchicken':  // dead animals can be cooked whole
            case 'rawdeermeat':
            case 'rawwolfmeat':
            case 'rawchickenmeat':  // A butcher shop will increase yield on meats (and cooking time)
    ...
    nextoutput() {
        ...
        if (this.outputfood !== 0 && this.onhand.length !== 0) {
    ...
    update() {
        // Xetera: This function is WAY too long, a lot of what you have here are things you repeat later on.
        // Xetera: Breaking things down to smaller function seems useless but you usually end up needing them later
                    ...
                    if (this.counter > -30) { // make sure this food isn't burnt
                        switch (this.foodin[0].name) {
                            case 'deaddeer':  // deer meat. provides the most amount of food. In this form it provides a little less meat than if it were butchered before cooking
                                ...
                                // Xetera: Why not just use a loop? Doesn't it feel off when you repeat yourself this much?
                                var food = new item('deermeat');
                                food.lifetime = 600;
                                food.location = this.id;
                                foodlist.push(food);
                                this.onhand.push(food);  // 600 = 10 minutes
                                var food = new item('deermeat');
                                food.lifetime = 600;
                                food.location = this.id;
                                foodlist.push(food);
    ...
    cooktime(itemname) {
        ...
        switch (itemname) {
            ...
            case 'deadchicken':
                return 50;
            case 'rawdeermeat':
                return 10;  // The butchered meats will all be relatively the same size, so have the same cooking time.  These may seem short, but
            case 'rawwolfmeat':
                return 10;  // it will help support larger populations
            case 'rawchickenmeat':
                return 10;
        }
    }

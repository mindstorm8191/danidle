class claydryer extends activeblock {
    ...
    possibleoutputs(askedlist) {
        ...
        var chunk = [];
        // Xetera: for (let i in this.stored) is a much cleaner way of doing this
        for (var i = 0; i < this.stored.length; i++) {
            switch (this.stored[i].name) {
                case 'dirtbrick':
                    chunk.push('dirtbrick');
                    break;
                default:
                    console.log('Error in claydryer->possibleoutputs: item type of "' + this.stored[i].name + '" not yet supported - please add');
            }
        }
        return chunk;
    }

class firewood extends activeblock {
    ...
    receiveitem(item) {
        ...
        if(item.name!='log') {
            console.log('Error in firewood->receiveitem: item ('+ item.name +') received, this only accepts logs.');
        }
        if(this.log.length>=10) {
            console.log('Error in firewood->receiveitem: input inventory full, cannot accept another item');
        }
        this.log.push(item);
    }
    ...
    update() {
        ...
        if(this.axe==null) {
            // No axe on hand.  Search for one now (if the user has selected one)
            this.axe = blocklist.findinstorage(this.targetaxe, 1);
        }
        if(this.log.length<1) {
            // No logs on hand.  Search for one now
            for(var i=0; i<4; i++) {
        ...
        if(this.counter<8) {
            // Not enough progress to produce anything
            return;
        }
        this.counter-=8;
    ...
    reload() {
        ...
        this.drawgameblock('img/firewood.png', 1);

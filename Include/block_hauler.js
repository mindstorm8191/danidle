class hauler extends activeblock {
    ...
    update() {
        ...
        if(workpoints<1) {
            return;  // if we cannot work, there's nothing else to do here
        }
                            ...
                            for(var k=0; k < this.targets[i].goingto.length; k++) {
                                ...
                                if(target==null) {
                                    ...
                                    if(target.acceptsinput({name: this.targets[i].name})==1) {
                                        ...
                                        $("#game").append('<div id="haulerimage'+ this.id +'" style="top:'+ (this.ypos*66+3) +'px; left:'+ (this.xpos*66+3) +'px; z-index:5; position:absolute; pointer-events:none;">'+
        ...
        if(this.status===1) {
            ...
            if(this.progress<=Math.abs(this.xpos-this.targetx)) {
                if(this.targetx>this.xpos) {
                    $("#haulerimage"+ this.id).css('left', ((this.xpos+this.progress)*66+3) +'px');
                }else{
                    $("#haulerimage"+ this.id).css('left', ((this.xpos-this.progress)*66+3) +'px');
                }
            }else{
                if(this.targety>this.ypos) {
                    $("#haulerimage"+ this.id).css('top', ((this.ypos+(this.progress-Math.abs(this.xpos-this.targetx)))*66+3) +'px');
                }else{
                    $("#haulerimage"+ this.id).css('top', ((this.ypos-(this.progress-Math.abs(this.xpos-this.targetx)))*66+3) +'px');
            }   }
            return;
    ...
    drawpanel() {
        ...
        $("#gamepanel").html('<center><b>Item Hauler</b></center><br /><br />'+
            'Employs a worker to carry an item from one location to another.  Choose a neighbor-block\'s output item below.  Press the plus key for that item, then click the '+
            'place on the map to take that item to. Items won\'t be moved unless source block has output AND target can accept it.<br />'+
    ...
    handlemaptrigger(targetblock, gridx, gridy) {
        ...
        if(targetblock==null) {
            console.log('Error in hauler->handlemaptrigger: unexpected targetblock is null');
            return;
        }
    ...
    updatepanel() {
        ...
        if(this.onhand==null) {
            $("#sidepanelholding").html('none');
            return;
        }
        $("#sidepanelholding").html(this.onhand.name);
    }

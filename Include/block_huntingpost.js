...
//  receiveitem(item) { }
    ...
    update() {
        ...
        if(this.weapon==null) {
            // No hunting weapon on hand.  Load one (unless the user hasn't selected one)
        ...
        if(this.onhand.length>=10) {
            // No room for more ourput items
            return;
        }
        if(workpoints<1) {
            // No workpoints to run this block with
            return;
        }
        workpoints--;
        ...
        if(this.counter<30) {
            // Not enough progress to complete hunt
            return;
        }
        this.counter-=30;
        var newcatch = null;

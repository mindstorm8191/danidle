class postmaker extends activeblock {
    ...
    receiveitem(item) {
        ...
        if(item.name!='log') {
            console.log('Error in postmaker->receiveitem: item ('+ item.name +') received, this only accepts logs.');
            return;
        }
        if(this.log.length>=10) {
            console.log('Error in postmaker->receiveitem: inventory is already full (level='+ this.log.length +')');
            return;
        }
        this.log.push(item);
    }
    ...
    // activeblock function that allows any internal processes to be carried out, once per tick.  This is called from a 'global' position
        ...
        if(this.onhand.length>=15) {
            // Not enough output inventory to make more
            return;
        }
        if(workpoints<1) {
            // Not enough worker time to work here
            return;
        }
    ...
    reload() {
        ...
        if(this.axe!=null) {
            Object.setPrototypeOf(this.axe, item.prototype);
        }

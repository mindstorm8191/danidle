...
//  receiveitem(item) { } This shouldn't be called
    ...
    nextoutput() {
        ...
        if(this.onhand.length<=0) {
            // We don't have anything on hand anyway
            return;
        }
    ...
    update() {
        ...
        if(this.pickaxe==null) {
            // We don't have a pickaxe loaded.
            if(this.targetpickaxe=='') {
                return;  // We don't have any pickaxe selected anyway.
            }
            this.pickaxe = blocklist.findinstorage(this.targetpickaxe, 1);
            return;
        }
        if(this.onhand.length>=5) {
            return;  // We don't have room for any more output items
        }
        if(workpoints<1) {
            return;  // We don't have any workers to do this job
        }
        workpoints--;
    ...
    reload() {
        ...
        if(this.pickaxe!=null) {
            Object.setPrototypeOf(this.pickaxe, item,prototype);
        }
